...
# Install packages required for the analysis (don't run this if packages are already downloaded)
install.packages(c('lmtest', 'Epi', 'tsModel', 'vcd'))

# load the packages
library(foreign) ; library(tsModel) ; library(lmtest) ; library(Epi)
library(splines) ; library(vcd)

# you can read data in from a csv file using the code below
# uncomment it if you have your data stored locally on your machine

#data <- read.csv("sicily.csv")

# we will read data in from a web url 
data <- read.csv('https://raw.githubusercontent.com/generationai/test-github-pages/master/sicily.csv')
head(data)
View(data)
